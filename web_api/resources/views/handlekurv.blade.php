...
@section('content')
    ...
    @foreach ($varer as $vare)
        <h4><img src="{{ Storage::disk('public')->url('thumb/'.$vare->varedata['bilde']) }}" style="max-height: 50px;"> {{ $vare->antall }} stk - {{ $vare->varedata['vare_navn'] }} <small> ({{ $vare->varedata['vare_beskrivelse'] }}) </small> &nbsp;  &nbsp; {{ ($vare->antall * $vare->varedata['pris']) }} <small>kroner</small></h4>
    @endforeach
    <hr>
